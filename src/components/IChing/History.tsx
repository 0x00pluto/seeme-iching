        ...
        {items.sort((a, b) => b.timestamp - a.timestamp).map((item) => {
          ...
          return (
            <motion.button
              ...
              className="flex items-center gap-6 p-6 rounded-[32px] border border-ink/5 bg-white/40 backdrop-blur-sm hover:bg-white/60 hover:border-brand/20 transition-all text-left shadow-sm hover:shadow-xl group"
            >
              <div className="w-16">
              ...
              <div className="flex-1 flex flex-col gap-1 overflow-hidden">
                ...
                <div className="text-xs text-brand font-serif tracking-widest">
                  {gua?.name} · {gua?.character}
                </div>
              </div>

              <ChevronRight size={20} className="text-ink/10 group-hover:text-brand group-hover:translate-x-1 transition-all" />
            </motion.button>
          );
        })}
      </div>
