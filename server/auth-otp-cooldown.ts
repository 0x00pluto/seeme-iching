...
export function checkSendCooldown(phoneE164: string): {
  allowed: boolean;
  resendAvailableAt?: string;
} {
  const last = lastSentAtByPhone.get(phoneE164);
  if (last == null) {
    return { allowed: true };
  }
...
export function recordSend(phoneE164: string): string {
  const now = Date.now();
  lastSentAtByPhone.set(phoneE164, now);
